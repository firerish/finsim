...
function run(startingPosition, targets, allocations, economy, events, statusCell) {

  statusCell.setBackground("#E0E0E0")
  statusCell.setValue("Initializing");

  let startingAge = startingPosition.currentAge;
  let targetAge = targets.finalAge;
  let initialSavings = startingPosition.currentSavings;
  let initialPension = startingPosition.retirementAccounts;
  let initialETFs = startingPosition.ETFs;
  let initialTrusts = startingPosition.equity;

  let retirementAge = targets.retirementAge;
  let emergencyStash = targets.targetSavings;
  let pensionPercentage = allocation.investments.Pension;
  let statePensionWeekly = spreadsheet.getRangeByName("StatePensionWeekly").getValue();

  let growthRatePension = economy.assetGrowth.Pension;
  let growthDevPension = economy.assetVariance.Pension;
  ...
  let growthRateTrust = economy.assetGrowth.Equity;
  let growthDevTrust = economy.assetVariance.Equity;
  inflation = economy.inflation;

  let etfAllocation = allocations.investments.ETFs;
  let trustAllocation = allocations.investments.Equity;

  let priorityCash = allocations.drawdownOrder.Cash;
  let priorityPension = allocations.drawdownOrder.Pension;
  let priorityEtf = allocations.drawdownOrder.ETFs;
  let priorityTrust = allocations.drawdownOrder.Equity;
  ...
  for (let i = 1; i <= Events.getHeight(); i++) {
    ...
    let valid = { "NOP": "NOP", "RI": "Rental Income", "SI": "Salary Income", "SIp": "Salary Income with Private Pension contribution", "UI": "RSU Income", "DBI" : "Defined Benefit Pension Income", "FI" : "Tax-free Income", "E": "Expense", "R": "Real Estate", "M": "Mortgage", "SM": "Stock Market" };
